/* ================================================
   Mechanomyography — Shared JS (api.js + auth.js + utils.js)
   Satu file, di-include di semua halaman
   ================================================ */
...
// ── API (offline — semua pakai localStorage, tanpa backend) ──
const API = {
  register: async (p) => {
...
function drawWave(canvas, color, offset) {
  ...
  ctx.strokeStyle = color; ctx.lineWidth = 1.8; ctx.shadowBlur = 10; ctx.shadowColor = color;
  ctx.beginPath();
  for (let x=0; x<W; x++) {
...
(function injectCSS() {
  ...
  l.rel='stylesheet'; l.href='https://fonts.googleapis.com/css2?family=DM+Serif+Display:ital@0;1&family=DM+Mono:wght@400;500&family=Outfit:wght@300;400;500;600;700&display=swap';
