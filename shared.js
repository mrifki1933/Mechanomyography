...
// ── KONFIGURASI ANTARES ──────────────────────────────────────
const ANTARES_KEY  = 'de3436ad8170bd02:79f135d091db42b7';
const ANTARES_APP  = 'mmg-sensor';
const ANTARES_DEV  = 'mmg-sensor';
const ANTARES_URL  = `https://platform.antares.id:8443/~/antares-cse/antares-id/${ANTARES_APP}/${ANTARES_DEV}/la`;
// /la = latest (data terbaru)

// ── MODE ──────────────────────────────────────────────────────
// true  = pakai data simulasi (sebelum sensor fisik siap)
// false = pakai data real dari Antares / ESP32
...
// ── ANTARES — Fetch data terbaru dari sensor ─────────────────
const Antares = {
  /**
   * Ambil data terbaru dari Antares (dikirim ESP32)
  ...
  fetchLatest: async () => {
    ...
    try {
      // Pakai CORS proxy agar bisa diakses dari Vercel / browser
      const PROXY   = 'https://corsproxy.io/?';
      const fullURL = PROXY + encodeURIComponent(ANTARES_URL);
      ...
      // Data ada di json['m2m:cin']['con']
      const con = json['m2m:cin']?.con;
      if (!con) throw new Error('Format data tidak valid');

      // con bisa berupa string JSON, Python dict string, atau object
      let data;
      if (typeof con === 'string') {
        // Antares kadang kirim string format Python dict: {'key': value}
        // Perlu convert ke JSON valid dulu
        const jsonStr = con
      ...
      return {
        amplitude:   parseFloat(data.amplitude   || 0),
        frequency:   parseFloat(data.frequency   || 0),
        rms:         parseFloat(data.rms         || 0),
        grip_pct:    parseFloat(data.grip_pct    || 0),
        fatigue_pct: parseFloat(data.fatigue_pct || 0),
        kontraksi:   parseInt  (data.kontraksi   || 0),
        timestamp:   data.timestamp || Date.now(),
      };

    } catch (e) {
      console.warn('[Antares] Gagal fetch:', e.message);
      return null;
    }
  },

  /**
   * Mulai polling data dari Antares setiap intervalMs
  ...
  startPolling: (callback, intervalMs = 3000) => {
    console.log(`[Antares] Mulai polling setiap ${intervalMs}ms`);
    let lastTimestamp = 0;
    ...
    poll(); // langsung fetch sekali
    const timer = setInterval(poll, intervalMs);

    return () => {
      clearInterval(timer);
      console.log('[Antares] Polling dihentikan');
    };
  },
};
...
(function injectCSS() {
  ...
  badge.innerHTML = '<div class="adot"></div><span id="antares-label">Antares terhubung</span>';
